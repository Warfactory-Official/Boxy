...
/**
 * Native server implementation of the platform-agnostic {@link OffThreadProcessor}: routes incoming requests to
 ...
 */
public class ServerOffThreadProcessor extends OffThreadProcessor<PlayerRequestState, ChunkDiskReader.ReadResult> {
    ...
    public ServerOffThreadProcessor(
    ...
    @Override
    protected void submitDiskRead(UUID playerUuid, int requestId, String dimension, int cx, int cz, long submissionOrder, boolean forceFresh) {
        if (this.diskReader != null) {
            ServerLevel level = this.dimensionLevelMap.get(dimension);
            if (level != null) {
                this.diskReader.submitReadDirect(playerUuid, requestId, level, cx, cz, submissionOrder, forceFresh);
            }
        }
    }
    ...
    protected void buildAndEnqueueColumnPayload(
        ...
        ResourceKey<Level> dimensionKey = this.dimensionKeyCache
                .computeIfAbsent(dimension, d -> ResourceKey.create(Registries.DIMENSION, ResourceLocation.parse(d)));
