...
4. **Boxy's own mixins live in two places.**
   ...
   - **In Boxy's own `boxy.mixins.json`** (shipped in the standalone mod, §8): the VSS server hooks `AccessorChunkMap` + `ChunkMapSaveHook` + the edit-gated `MixinServerLevelLiveDirty` (§9); the distant-entity server hook `MixinChunkMapTrackedEntity`; and the client-side distant-entity mixins `AccessorClientLevel`, `MixinClientPacketListenerServerPos`, `MixinEntityRenderDistance`, `MixinLevelRendererDistantEntities`, and the ReplayMod-compatibility guard `MixinEntityForeignTickGuard` (all §11); and the Voxy render-pipeline compat mixins `MixinVoxyDepthStateRestore` (Oculus depth-state re-sync, §12) and `MixinVoxyStencilMaskFix` (restores the GL stencil write mask so a mod like ModularUI can't no-op Voxy's LOD-mask stencil clear, quirk 41); the Voxy performance mixins `MixinVoxyModelBakeryWake` + `MixinVoxyMapperBiomeCache` (§12 *Voxy performance mixins*); and the Voxy 0.2.17-beta backport mixins `MixinVoxyMipperLightFix`, `MixinVoxyInstanceShutdownFix`, `MixinVoxyModelFactoryFixes` + `AccessorVoxyModelBakeUpload`, and `MixinVoxyRenderDataFactoryEmissive` (§12 *Voxy backport mixins*). This config is Boxy's own and never touches Voxy's.
...
com/golem/boxy/vss/
  ...
  client/              LodRequestManager, SpiralScanner, ClientNetworking, ClientColumnProcessor,
                       ...
                       (+ ClientEntitySync, DistantEntityTicker, DistantEntityServerPos,
                        DistantEntityDepthFix, VoxyLodDepthJoin — distant entities, §11)
  ...
  mixin/               VSS: AccessorChunkMap (@Accessor level), ChunkMapSaveHook (@Inject save),
                       ...
                       MixinLevelRendererDistantEntities, MixinEntityForeignTickGuard (ReplayMod compat) (client).
...
A fifth, defensive lift exists alongside gate 2: third-party entity cullers (e.g. Sodium/Embeddium Extra's entity-distance option) veto entities deeper in the chain than gate 2 can reach, so `MixinLevelRendererDistantEntities` also wraps `renderLevel`'s `EntityRenderDispatcher.shouldRender` call and re-runs the distance + frustum decision for tracked types a foreign mod culled (quirk 48).
...
### Distant-entity depth precision (`DistantEntityDepthFix`) — on by default
At LOD-scale distances the standard depth buffer stops being able to separate a model's overlapping surfaces: it's 24-bit fixed-point under a projection with near = 0.05, which crams everything beyond ~256 blocks into the last ~0.02% of the depth range — one depth step ≈ z²/(0.05·2²⁴) blocks (~0.3 blocks at 512), far coarser than the 0.03–0.06-block gaps between model layers (skin overlays, armor, vehicle panels). Layer pairs land on the same or randomly adjacent depth values and shimmer as the camera moves (quirk 44). A global cure (reversed-Z / float depth) can't be retrofitted under MC + Voxy + Embeddium, but a distant entity only occupies a **thin slice of eye-space depth** `[zc − slab, zc + slab]` — so Boxy gives that slice its own precision. A second wrap in `MixinLevelRendererDistantEntities` (on the `EntityRenderDispatcher.render` call inside `LevelRenderer.renderEntity`, SRG `m_109517_` → `m_114384_`) diverts a qualifying entity's draws into a private immediate buffer and flushes it with **(a)** the projection's **z row rebuilt** so near/far tightly bracket the slab — the entity's depth then spans the full NDC range at full float precision instead of the last few ulps below 1.0; x/y rows are untouched, so the screen-space footprint is bit-identical — and **(b)** `glDepthRange(wLo, wHi)` set to the exact window-depth band the slab occupies under the *real* projection (computed in double). The fixed-function viewport transform maps the well-spread NDC monotonically into that band, so the entity gets every depth step the band physically has (hundreds at typical distances — full internal ordering), sub-step layer pairs collapse into *stable* draw-order ties (outer layers draw after their base, which is the correct result), and compositing against Voxy LOD terrain / vanilla terrain / other entities is preserved because the band endpoints are the true depth values of the slab bounds. The modelview is untouched (fog, lighting, animation unchanged). It also applies under an **Oculus shaderpack** (experimental): Iris's patched gbuffer entity programs are `ShaderInstance`s whose `ProjMat` uniform is uploaded per draw from the *live* `RenderSystem` projection, so the z-row swap reaches them — the frame-captured `gbufferProjection` is only used by composite passes, and since every written depth value is a valid point on the global depth curve inside the entity's true band, composites that reconstruct position from depth (deferred fog/shadows/SSR) decode within the slab of the truth; Iris's shadow pass is orthographic and fails the perspective check, so it flushes plainly. **Stand-downs** (each falls back to a plain flush or the vanilla path — rendering never breaks): closer than 64 blocks; the glowing-outline buffer source; a non-perspective projection (covers shadow passes); a bounding box too large relative to the distance; a degenerate band (entity wholly past the far plane). The client knob `distantEntityDepthMode` (`DistantEntityDepthMode`, default **`PRECISE`**, read live, cycling control in the Embeddium **Boxy** page) has three levels: **OFF**; **BASIC** = one pass, the band is exactly the true one (compositing exact, but a thin slab only holds ~30 depth steps at 32 chunks, so ~0.1–0.3-block gaps can still flicker at long range); **PRECISE** = compositing-exact **and** the full 24 bits of depth across the slab, by splitting the depth test's two jobs into passes: (1) a color-masked **mask pass** with the exact band writes true depth and sets stencil bit 7 where fragments pass (a pixel-exact visibility mask, decided against terrain at true depth); (2) a stencil-gated fullscreen quad **clears depth to 1** inside the mask; (3) the **fine pass** draws the entity color-on, stencil-clipped, with the tight projection across the full 0–1 depth range (essentially perfect internal ordering); (4) a **restore pass** (color off, depth ALWAYS, exact band) rewrites true depth inside the mask and zeroes the stencil bit (`GL_ZERO` masked write — idempotent per fragment) so water/particles/later entities composite correctly. Bit 7 masked writes never touch Voxy's LOD-mask bit (quirk 41 — that lives in Voxy's *own* framebuffer anyway), and the color/depth-func overrides use **raw GL** so `GlStateManager`'s cache makes the RenderType shards' redundant state sets no-ops. The multi-pass needs a **stencil attachment on MC's main render target**, which doesn't exist by default (quirk 45): `DistantEntityDepthFix.ensureStencil()` (called from `DistantEntityTicker`'s client tick, safely between frames since `enableStencil()` recreates the framebuffer) turns it on while the mode is PRECISE; until then the multi-pass holds back on the widened-band fallback. Occlusion against LOD terrain works because Voxy's final composite (`blit_texture_depth_cutout.frag`) reprojects its LOD depth through MC's projection and writes `gl_FragDepth` into MC's depth buffer during the terrain stage (clamped to just below 1.0 past the far plane) — the mask pass tests against exactly that. Under a shaderpack that blit is conditional and often absent, so Boxy performs its own once-per-frame LOD depth join there (`VoxyLodDepthJoin`, quirk 46). Cost: each distant entity's geometry is built ~3× + one stencil-gated quad — trivial for the handful of tracked entities on screen. Under an active shaderpack PRECISE **degrades to BASIC** (exact band, single pass): the multi-pass needs a stencil attachment, Iris's shared G-buffer depth texture has none (and offers no way to request one), and its per-program framebuffer binding hides the real render target — an earlier widened-band fallback was tried there and dropped (it reintroduced both ridge show-through *and* layer shimmer, the worst of both). One-time diagnostic `Boxy distant-entity depth re-band ACTIVE`. Depth aside, residual shimmer at these scales is texture/geometry aliasing (sub-pixel models, no MSAA) — a texture-mipmap option was tried for that and removed (quirk 29).

### Client settings GUI (`BoxyConfigPage`) — a page in Embeddium's Video Settings
Mirroring Voxy's own config page, the client mixin `MixinEmbeddiumOptionsGUI` appends a **Boxy** `OptionPage` to Embeddium's `SodiumOptionsGUI` at construction (`<init>` TAIL inject, `remap = false`, `require = 0` — the same hook Voxy uses; both injectors coexist). `BoxyConfigPage` builds the page with Embeddium's `OptionImpl`/`OptionGroup`/`SliderControl`/`TickBoxControl` API, bound to `VSSClientConfig.CONFIG` through `BoxyOptionStorage` (an Embeddium `OptionStorage<VSSClientConfig>` whose `getData()` returns the config and `save()` calls `VSSClientConfig.saveClamped()` = clamp + write JSON). It surfaces every client knob below **except** `renderedEntityTypes` — Embeddium has no list control, and the server's synced list is authoritative anyway, so that one stays JSON-only. All knobs are read live. Everything Embeddium-facing is **client-only** (the mixin lives in `boxy.mixins.json`'s `client` list; `me.jellysquid.*` is absent on a dedicated server, so these classes never load there) and compiles against Embeddium from Maven (`maven.modrinth:embeddium`, compile-only; §15).
...
| `distantEntityDepthMode` | `PRECISE` | Distant-entity z-fighting fix: `OFF` / `BASIC` (one pass, depth exact) / `PRECISE` (multi-pass, exact occlusion + full 24-bit internal depth; behaves like `BASIC` under shaderpacks). |
...
- **Voxy backport mixins (0.2.17-beta fixes).** Upstream Voxy moved on to 0.2.17-beta (MC 26.x); Boxy backports its engine-level fixes into the 0.2.14 build as **reimplementations** (no Voxy code is shipped) — all string-target Voxy classes with `remap = false` / `require = 0` (degrade to 0.2.14 behaviour), all in `boxy.mixins.json`'s client list, sharing the helper `vss.client.VoxyBackports`. **`MixinVoxyMipperLightFix`**: 0.2.14's `Mipper.mip` double-shifts the averaged block light so every mip level truncates it to garbage (`&0xFF`) — distant LODs lose torch/lava/glowstone light; a redirect of the single `Mapper.withLight` call recovers the correct byte from the pre-mask value. **`MixinVoxyInstanceShutdownFix`**: `VoxyInstance.shutdown` busy-waits on a still-used world *while holding `activeWorldLock`'s write lock* (hang on quit/world switch); an inject before the `writeLock()` call drains used worlds lock-free first (60 s cap → falls back to original behaviour). **`MixinVoxyModelFactoryFixes`** (+ `AccessorVoxyModelBakeUpload`): (1) a missing registry biome in `processAllThings` (`getOptional(...).orElseThrow()`) crashed the bake thread when a biome mod/datapack was removed — now warns and substitutes plains; (2) translucent-declared bakes whose texels aren't actually translucent are reclassified to solid/cutout (`@ModifyArgs` on the `processTextureBakeResult` call), fixing wrong-layer LOD artifacts; (3) at bake RETURN it writes the state's light emission into metadata bits 55-58 (unused in 0.2.14; the bits upstream's `ModelQueries.lightEmission` defines) and registers biome-coloured models baked before any biome was known into `modelsRequiringBiomeColours` (0.2.14 skipped them → permanently untinted grass/leaves/water for that model). **`MixinVoxyRenderDataFactoryEmissive`**: the mesh-side half of emissive LODs — redirects all 15 `Mesher.putNext` emission sites (the same set upstream wraps with `applyQuadLight`) and raises each quad's block-light nibble (bits 59-62) to the model's own emission, resolved from the quad's client-model-id bits (26-41). This is a per-emitted-quad hot path on the mesh workers (shared with Embeddium's chunk builders), so the instance-method sites read the shadowed `modelMan` field directly; only the two static helpers resolve via a ThreadLocal `ModelFactory` set at `generateMesh` HEAD (never cleared — `IdNotYetComputedException` unwinds make RETURN unreliable; it's refreshed every entry). **Mixin gotcha encoded here:** `generateMesh` returns `BuiltSection`, so its HEAD-inject callback must be `CallbackInfoReturnable` — a plain `CallbackInfo` is a *descriptor validation* error that fails the whole mixin at apply time **despite `require = 0`** (unlike a missing target, which degrades) and killed every mesh worker with `MixinTransformerError` when first shipped wrong. One accepted race: emission metadata lands just *after* `idMappings` publication, so a mesher in that microsecond window builds one section unglowed until its next rebuild. There is also one **remap-time asset patch** (v38): `assemble()` rewrites `getLightmapUv` in `assets/voxy/shaders/lod/lighting.glsl` to upstream's texel-center mapping (`base*(15/16)+0.5/16`) — pattern-matched byte-exact, copied unpatched with a warning if Voxy's shader ever changes. *Backports evaluated but intentionally skipped:* upstream's `requestBlockBake`/`addEntry` ordering-race fix (unreachable on 0.2.14 — every producer is serialized through `ModelBakerySubsystem.enqueueLock`), the fixed-point `SoftwareRasterizer` rewrite (wholesale class replacement), and everything reversed-Z/`RenderProperties` (new-MC pipeline, impossible on 1.20.1).
...
- **`REMAP_VERSION`.** A string constant in `VoxyRemapper` (currently **`v38`** — check the constant, this guide can lag). It's part of every cache filename. **Bump it whenever you change any remap/patch/classpath logic**, or stale cached jars will be reused.
...
| 29 | Distant entity textures garble/shimmer when zoomed | Entity textures load with 0 mip levels and entity RenderTypes filter NEAREST/no-mipmap → aliasing under heavy minification | An optional `mipmapEntityTextures` (alpha-aware mip chain + `NEAREST_MIPMAP_LINEAR`) was shipped, then **removed** — in-game testing showed no visible improvement at these distances. Currently unaddressed |
...
| 43 | With **Radium** installed (quirk 42's alias working, no errors logged anywhere): flying/moving away from spawn, **vanilla chunks around the player intermittently never load client-side** — no render, no collision ("finicky physics") — and Voxy paints LOD terrain into the holes, which presents as "LODs rendering over real chunks / culled wrongly / LODs stuck at spawn". Initially misread as a Voxy/Boxy freeze | Bisected in-game with staged pipeline counters (temporary `MixinVoxyDiag*` mixins, since removed): every Voxy stage measured **healthy** under Radium — ingest/palette-convert (quirk 42 fast-path fine), meshing, `AsyncNodeManager` GPU sync, node window tracking, and the GPU cull was actively drawing millions of quads. The defect is upstream of Voxy entirely: Radium's `mixin.world.player_chunk_tick` optimization (Lithium's `ThreadedAnvilChunkStorageMixin`, replacing vanilla's player chunk-send tracking) drops chunk **delivery** to moving players on this stack. Voxy is just the paint that makes the missing chunks visible | Setting `mixin.world.player_chunk_tick=false` in `config/lithium.properties` (Radium reads Lithium's config format) fixes it while keeping the rest of Radium. `BoxyModLocator.ensureRadiumPlayerChunkTickDisabled()` now writes that key automatically at locate time when a Lithium-port jar (marker `lithium.mixins.json`) is in mods — before Radium's mixin plugin reads the file; an existing explicit setting for the key (either value) is respected. Not a Voxy/Boxy bug — no remap or mixin change, `REMAP_VERSION` unchanged |
| 44 | Tracked entities **z-fight badly at distance** — layers/armor/vehicle panels shimmer past ~8–12 chunks, severe by 32 (mipmapping doesn't help; it's not texture aliasing) | 24-bit fixed-point depth + near plane 0.05 cram everything past ~256 blocks into the last ~0.02% of the depth range: one depth step ≈ z²/(0.05·2²⁴) blocks (~0.3 at 512), coarser than the 0.03–0.06-block gaps between model layers, so layer pairs land on the same or randomly adjacent depth values. The global cure (reversed-Z / float depth) can't be retrofitted under MC + Voxy + Embeddium | **`DistantEntityDepthFix`** (§11): the `EntityRenderDispatcher.render` wrap in `MixinLevelRendererDistantEntities` diverts a distant tracked entity's draws to a private buffer and flushes it with the projection z-row rebuilt to tightly bracket the entity's depth slab + `glDepthRange` set to the true window-depth band of that slab — full local depth resolution, exact compositing at the band bounds, x/y projection rows untouched. Applies under Oculus shaderpacks too (live `ProjMat` feeds Iris's patched entity programs; experimental); stands down for glowing outline / huge bounding boxes / non-perspective projections (incl. shadow passes). Client knob `distantEntityDepthMode` (`OFF`/`BASIC`/`PRECISE`, default `PRECISE`; `PRECISE` = multi-pass mask/clear/fine/restore render for exact occlusion **plus** full 24-bit internal depth, behaving like `BASIC` under shaderpacks — no stencil there); no remap change, `REMAP_VERSION` unchanged |
| 45 | With the PRECISE depth mode (quirk 44), distant entities **x-ray through everything** — Voxy LODs and vanilla terrain — instead of being occluded | MC's main framebuffer has **no stencil attachment** by default (Voxy's stencil work is in its own FBO). Per the GL spec, without an attachment the stencil test silently always-passes and stencil writes no-op — so the mask pass stored nothing and the "stencil-gated" depth-clear quad wiped the **entire frame's depth** to 1.0, letting every later draw pass its depth test | `DistantEntityDepthFix.ensureStencil()`: enable the main target's stencil (Forge's `RenderTarget.enableStencil()`) from the client tick — **between frames only**, since it recreates the framebuffer — while the mode is PRECISE; the multi-pass additionally gates on `isStencilEnabled()` and falls back to the exact-band single pass until it's on |
| 46 | Under an **Oculus shaderpack**, distant tracked entities show through Voxy LOD terrain (all depth modes; predates the depth re-band) | Voxy's **Iris** pipeline only reprojects LOD depth into the vanilla depth buffer when `IrisVoxyRenderPipelineData.renderToVanillaDepth` is true **and** the target size matches the viewport — a pack's voxy patch can set `excludeLodsFromVanillaDepth`, and a render-scale mismatch skips it silently. Entities then depth-test against a buffer that never learned the LOD terrain. (The vanilla `NormalRenderPipeline` always blits — quirk 45's §11 note — which is why the vanilla path occludes fine) | **`VoxyLodDepthJoin`**: once per frame (deduped on `Viewport.frameId`), before the first distant-entity flush under an active pack, Boxy reprojects Voxy's LOD depth (`IrisVoxyRenderPipeline.fbTranslucent`) into the bound depth buffer itself — a raw-GL fullscreen pass mirroring Voxy's own `blit_texture_depth_cutout.frag` math (inverse Voxy MVP → `vanillaProjection × modelView`), color-masked off with `LEQUAL` (an idempotent min-join, harmless when Voxy already blitted). Raw GL + full save/restore keeps Iris and `GlStateManager` oblivious; the one private touch (`VoxyRenderSystem.pipeline`) is a cached reflective read; any failure disables the join for the session with a warning |
| 47 | With the **blindness** (or darkness) effect, Voxy LODs stay visible through the near-black fog that hides everything else | Vanilla's effect fog closes a few blocks out; Voxy's vanilla pipeline bails on its own (its composite checks the captured fog range), but the **Iris** pipeline renders LODs through the shaderpack, and packs don't apply blindness fog to Voxy's LOD program | A second injector in `MixinVoxyDepthStateRestore`: `@Inject` HEAD `cancellable` on `AbstractRenderPipeline.runPipeline`, cancelling the whole LOD pipeline while the local player `hasEffect` blindness/darkness (safe — `runPipeline` touches no GL state before its own setup; `require = 0`) |
| 48 | With **Sodium/Embeddium Extra** installed, distant entity rendering silently stops working | Its entity-distance culling injects deeper in the chain (`EntityRenderer.shouldRender`) and vetoes far entities before Boxy's distance gate (gate 2, `Entity.shouldRenderAtSqrDistance`) is ever consulted | Safety-net `@WrapOperation` in `MixinLevelRendererDistantEntities` on `renderLevel`'s `EntityRenderDispatcher.shouldRender` call (`m_114397_`): when it returns false for a tracked type with the feature on, re-run the decision vanilla would have made — Boxy's extended distance cull + the real frustum test (`getBoundingBoxForCulling().inflate(0.5)`, honoring `noCulling`) — overriding any foreign distance veto without breaking frustum culling. One-time diagnostic `Boxy re-allowed a distant tracked entity…` |
...
| `client.*` | Spiral request manager + queues + metrics, client networking, column processor, disk cache, `VoxyClientBridge` (the Voxy ingest call); `ClientEntitySync`, `DistantEntityTicker`, `DistantEntityServerPos`, `DistantEntityDepthFix`, `VoxyLodDepthJoin` (distant entities, §11); `BoxyConfigPage` + `BoxyOptionStorage` (the Embeddium client-settings page, §11). |
...
| `mixin.*` (Boxy's own `boxy.mixins.json`) | VSS server hooks `AccessorChunkMap` + `ChunkMapSaveHook` + `MixinServerLevelLiveDirty` (§9). Distant entities (§11): `MixinChunkMapTrackedEntity` (server); `AccessorClientLevel`, `MixinClientPacketListenerServerPos`, `MixinEntityRenderDistance`, `MixinLevelRendererDistantEntities`, `MixinEntityForeignTickGuard` (client). Shaders (§12): `MixinVoxyDepthStateRestore` (string-targets Voxy's `AbstractRenderPipeline.runPipeline`, re-syncs GL depth-test state after Voxy's pipeline so Oculus's cloud/weather passes don't punch through terrain). Mod compat: `MixinVoxyStencilMaskFix` (string-targets `AbstractRenderPipeline.initDepthStencil`, forces `glStencilMask(0xFF)` before Voxy's LOD-mask stencil clear so a mod that left the write mask at `0x00` — e.g. ModularUI — doesn't silently no-op it and cull all LODs, quirk 41). Client settings GUI (§11): `MixinEmbeddiumOptionsGUI` (appends Boxy's page to Embeddium's `SodiumOptionsGUI`). Voxy 0.2.17-beta backports (§12 *Voxy backport mixins*): `MixinVoxyMipperLightFix` (mip-chain block-light corruption), `MixinVoxyInstanceShutdownFix` (shutdown deadlock), `MixinVoxyModelFactoryFixes` + `AccessorVoxyModelBakeUpload` (missing-biome crash, translucent-layer reclassification, emission metadata, biome-tint registration), `MixinVoxyRenderDataFactoryEmissive` (emissive quad light; shares `vss.client.VoxyBackports`). *(The old `MixinIrisCustomUniforms` was deleted — the bundled Mixin upgrade made it unnecessary, §12/quirk 36.)* |
...
GitHub Actions builds the production jar on every push and PR. It has no access to a local `libs/` dev jar, so it **builds Voxy's dev jar from source** ([m3t4f1v3/voxy](https://github.com/m3t4f1v3/voxy), ref pinned by `VOXY_REF`, JDK 21 for Loom while Boxy itself builds on JDK 17) and caches it keyed on the resolved Voxy commit. The workflow also runs the quirk-37 manifest check automatically (MANIFEST.MF first file entry + `Automatic-Module-Name: boxyloader`) and fails the build if it regresses. Pushing a `v*` tag publishes a GitHub Release with the jar, passing the tag version via `-Pmod_version`; tags containing a hyphen (e.g. `v1.0.0-alpha`) are marked pre-release.
...
- **Depth re-band (default on):** a tracked entity ~16–32 chunks out no longer z-fights — no shimmering/flickering between its model layers as the camera moves — and still sits correctly *on* and *behind* Voxy LOD terrain (walk it behind an LOD hill: it's occluded). Client log: `Boxy distant-entity depth re-band ACTIVE`. Under an Oculus shaderpack the fix applies too (experimental) — verify distant entities render at the right position and depth under the pack. The **Distant entity depth fix** control in the Boxy page cycles Off / Basic / Precise live; in Precise (vanilla pipeline), all shimmer is gone **and** occlusion stays exact (multi-pass) — also verify water/particles in front of a distant entity still draw over it (the depth-restore pass at work), and that Voxy LODs still render (stencil-bit interplay, quirk 41).
- **Shaders (Oculus, §12):** at boot the log shows **Mixin Transmogrifier** upgrading Mixin to the Fabric `0.12.11` fork, and the client reaches the title screen **without** the `module … reads more than one module named boxy` crash (quirk 37 — if it crashes there, re-run the post-build manifest check above). With **Oculus** installed the client log shows `Boxy: found Oculus (Iris) at … — enabling shader-pack support` and the cached jar is `voxy-srg-<REMAP_VERSION>-iris-<hash>.jar`. Enable a **voxy-aware** shaderpack (e.g. Complementary Reimagined/Unbound, which ship `shaders/world*/voxy*.glsl`) in Oculus's settings → it loads **without** the `InvalidInjectionException` on `iris.MixinProgramSet`/`MixinIrisRenderingPipeline` (quirk 35), the Voxy LOD section shaders compile (**no** `'vxModelView' : undeclared identifier` / `uniforms could not be found` spam — quirk 36), and LODs render lit/shadowed under the pack **with accurate lighting and no black band** (Voxy logs `Creating voxy iris render pipeline`). **Confirmed:** with the manifest fix in place this matches a Sinytra Connector + Embeddium + Oculus instance (the ground-truth reference). Without Oculus, LODs render unshaded exactly as before. Note a non-voxy-aware pack simply renders LODs unshaded (no `voxy.json` → Voxy uses its normal pipeline).
...
- **Distant entity textures still alias.** Sub-pixel models with NEAREST-filtered textures shimmer under heavy minification; a texture-mipmap option was tried and removed after testing showed no visible benefit (quirk 29).
- **Distant-entity z-fighting is fixed by the depth re-band; shaderpack support is experimental.** Vanilla entities share Minecraft's standard 24-bit depth buffer (Voxy renders its LOD with `glDepthFunc(GL_ALWAYS)` + its own HiZ occlusion, then restores `GL_LEQUAL` — so there's no reversed-Z to join), whose precision degrades with distance² — a detailed model past ~8–12 chunks used to fight for depth and shimmer (quirk 44). The per-entity depth re-band (`DistantEntityDepthFix`, §11; `distantEntityDepthMode`, default `PRECISE`) restores full local depth resolution without touching the global pipeline, and also engages under Oculus shaderpacks (the live `ProjMat` feeds Iris's patched gbuffer entity programs — §11); a pack whose entity shaders transform positions unusually could misrender distant entities, in which case the knob turns it off live. `distantEntityDepthMode = PRECISE` removes `BASIC`'s residual mid-gap flicker entirely via the multi-pass render (§11); under a shaderpack it behaves like `BASIC` — the multi-pass needs a stencil buffer Iris's G-buffer depth doesn't have, so some layer shimmer remains there. Glowing (outlined) entities keep vanilla depth, and an entity wholly past the vanilla far plane (`renderDistance × 4`) is still far-plane-limited, as before.
