...
- **Boxy hot-path performance rules.** Boxy's own hooks sit on some of the hottest paths in the game, and the code encodes a few invariants worth preserving. **(a)** The one-time `diag*` methods in `TrackedEntityTypes` are called from per-entity-per-tick/frame handlers, so each `compareAndSet` is guarded by a plain `get()` — a failed CAS still executes a hardware CAS; keep the guard when adding diagnostics. **(b)** `MixinEntityForeignTickGuard` (every entity tick) checks `ClientEntitySync.enabled()` first — its guards are order-independent early-returns, so the feature gate must stay the cheapest, earliest exit. **(c)** The VSS client spiral (`SpiralScanner`) no longer restarts from ring 0 on every event: chunk moves `recenter(moved)` (confirmed rings stay closed, shifted by the distance moved), dirty broadcasts re-open only the innermost dirty column's ring (`lowerConfirmedRing`), and **request timeouts must re-open their ring too** (`InFlightTracker.timeoutSweep` callback → `onRequestTimedOut`) — the old full restarts were what implicitly retried timed-out columns, so any new "this column needs another look" state must lower the confirmed ring itself. Prune passes over the timestamp/dirty/retry/validated collections run only after ≥8 chunks of movement (`PRUNE_HYSTERESIS_CHUNKS`; safe under `getPruneDistance`'s 32-chunk margin). **(d)** `DirtyColumnTracker.markDirty` (every block update server-wide via `MixinServerLevelLiveDirty`) memoizes the last-marked column and skips the monitor for repeat marks; the memo is invalidated in `drainDirty`, so keep that pairing if the drain path changes.
...
| `BoxyFabricLoaderImpl` | Backs the Fabric `FabricLoader` facade (synthetic mod container, reflective `isModLoaded` — with the `iris`→`oculus` alias for shader support, env/config dirs). |
